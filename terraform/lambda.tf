...
# ─────────────────────────────────────────────────────────────
# Lambda — xirr-processor function
# ─────────────────────────────────────────────────────────────
resource "aws_lambda_function" "xirr_processor" {
  function_name = "xirr-processor"
  description   = "XIRR calculator: parse ledgers, compute XIRR, generate PDF, send email"
...
resource "aws_lambda_function" "nifty_refresher" {
  function_name = "nifty-refresher"
  description   = "Downloads full Nifty 50 history from yfinance and caches in S3 daily"
  ...
  role        = aws_iam_role.lambda_exec.arn
  timeout     = 300   # up to 5 min for retries on yfinance rate limits
  memory_size = 256
